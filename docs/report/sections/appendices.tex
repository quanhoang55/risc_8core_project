...
\caption{Cấu trúc artifact của dự án}\label{tab:repo_tree}\\
\toprule
...
\midrule
\endhead
\texttt{rtl/core/} & ALU, control unit, PC logic, register file, lõi pipeline \\
\texttt{rtl/core/pipeline\_register/} & IF/ID, ID/EX, EX/MEM, MEM/WB, hazard controller \\
\texttt{rtl/interconnect/} & Arbiter Round-Robin và bus controller \\
\texttt{rtl/memory/} & Instruction ROM và data RAM \\
\texttt{rtl/top\_8core.v} & Top-level tám lõi \\
\texttt{software/} & Chương trình assembly và machine-code hex \\
\texttt{tb/} & Testbench, VVP/VCD sinh ra trong mô phỏng \\
\texttt{scripts/} & Script build hệ thống \\
\texttt{docs/report/} & Mã nguồn LaTeX và PDF báo cáo \\
\texttt{docs/slide/} & Slide và sơ đồ TikZ có thể tái sử dụng \\
\bottomrule
\end{longtable}
