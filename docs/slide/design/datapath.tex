...
\begin{frame}[fragile]{PC Logic: Kịch bản kiểm thử}
    ...
    \end{columns}
\end{frame}
\begin{frame}[fragile]{PC Logic: Kết quả mô phỏng \& Giản đồ sóng}
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
...
tb/pc_logic_tb.v:63: $finish called at 80000 (1ps)
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Giản đồ sóng (Waveform)}
                \centering
                \vspace{0.1cm}
                \includegraphics[width=\textwidth]{pc_logic_wave.png}
                \vspace{0.2cm}
                \scriptsize
                \begin{itemize}
                    \item \textbf{Stall (30-40ns):} PC giữ nguyên giá trị cũ (= 8).
                    \item \textbf{Branch (40-50ns):} PC nhảy tới `0x64` (= 100).
                    \item \textbf{Jump (50-60ns):} PC nhảy tới `0xC8` (= 200).
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}[fragile]{ALU: Kết quả mô phỏng Console}
    ...
    \begin{block}{Kết quả xuất ra màn hình (Console Output)}
        \begin{lstlisting}[basicstyle=\fontsize{6.0pt}{7.0pt}\selectfont\ttfamily]
...
Kiem tra ket thuc.
    ...
    \begin{itemize}
        \item \textbf{Tính toán chính xác:} Phép `ADD` (10 + 20 = 30) và phép `SUB` (50 - 50 = 0) cho kết quả khớp lý thuyết.
        \item \textbf{Cờ Zero:} Tự động kích hoạt lên `1` khi kết quả của phép `SUB` và `AND` bằng 0.
        \item \textbf{Phép toán có dấu SLT:} So sánh $-10$ và $5$ cho kết quả đúng bằng `1` (SLT thực hiện so sánh có dấu \texttt{\$signed}).
    \end{itemize}
\end{frame}
...
\begin{frame}{ALU: Giản Đồ Sóng Mô Phỏng (GTKWave)}
    \begin{block}{Giản đồ sóng chi tiết thu được từ bộ mô phỏng}
        \centering
        \vspace{0.05cm}
        \includegraphics[width=0.95\textwidth]{alu_wave.png}
        \vspace{0.05cm}
    \end{block}
    \vspace{0.15cm}
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item \texttt{alu\_control[3:0]}: Mã phép toán điều khiển (\texttt{2}=ADD, \texttt{6}=SUB, \texttt{0}=AND, \texttt{7}=SLT).
            \end{itemize}
        \end{column}
...
\begin{frame}{ALU: Phân Tích Chi Tiết Sóng Mô Phỏng}
    \begin{columns}
        \begin{column}{0.58\textwidth}
            \begin{block}{Phân tích 5 Test Case thực thi}
                \scriptsize
                \begin{itemize}
                    \item \textbf{0ns - 10ns (ADD):} $10 + 20 = 30$ (\texttt{0x1E}). Kết quả khác 0 $\rightarrow$ cờ \texttt{zero = 0}.
                    \item \textbf{10ns - 20ns (SUB):} $50 - 50 = 0$. Kết quả bằng 0 $\rightarrow$ cờ \texttt{zero = 1} kéo lên mức cao.
                    \item \textbf{20ns - 30ns (AND):} \texttt{0xAAAAAAAA} AND \texttt{0x55555555} = $0$. Cờ \texttt{zero} tiếp tục giữ ở mức cao.
                    \item \textbf{30ns - 40ns (SLT - Số dương):} So sánh $15 < 25$ (Đúng $\rightarrow$ \texttt{result = 1}). Cờ \texttt{zero = 0}.
                    \item \textbf{40ns - 50ns (SLT - Số âm):} So sánh có dấu $-10$ (\texttt{0xFFFFFFF6}) $< 5$ (Đúng $\rightarrow$ \texttt{result = 1}).
                \end{itemize}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.38\textwidth}
            \begin{block}{Nhận xét kỹ thuật}
                \scriptsize
                \begin{itemize}
                    \item Phép so sánh \texttt{SLT} hoạt động chính xác với số âm (\texttt{\$signed}) nhờ biểu diễn bù 2, không bị nhầm lẫn với số không dấu.
                    \item Cờ \texttt{zero} phản hồi tức thời theo giá trị của \texttt{result}.
                    \item Thiết kế ALU hoạt động đúng chức năng của tập lệnh RV32I.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\subsection{Cơ chế Pipeline và điều phối hệ thống}
